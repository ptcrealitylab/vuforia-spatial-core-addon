import {AFrameInterface} from '/objectDefaultFiles/AFrameInterface.js'
...
/**
 * code that mediates between the AFrameInterface, SpatialiInterface and the webworker
 */
class SimpleModelInterface {
    constructor() {
        console.log('tool is in a secure context: ' + isSecureContext + ' and isolated: ' + crossOriginIsolated);
        this.worker = new Worker('AFrameSimpleModelWorker.js', {type: 'module'});
        this.synclock = null;
		this.worker.onmessage = (event) => this.onMessageFromWorker(event);
        this.spatialInterface = new SpatialInterface();
        this.threejsInterface = new AFrameInterface(this.spatialInterface, this.worker);
